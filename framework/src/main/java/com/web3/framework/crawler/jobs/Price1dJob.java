...
public class Price1dJob {
    ...
    @Resource
    private Price1dService price1dService;
    @Resource
    ...
    private List<Price1d> addId(List<Price1d> price1dList) {
        ...
        List<Price1d> exsitsList = price1dService.list(wrapper);
        if (CollectionUtils.isEmpty(exsitsList)) {
            price1dService.saveOrUpdateBatch(price1dList);
            return price1dList;
        }
    ...
    public void execute() {
        ...
        price1dService.saveOrUpdateBatch(addId(price1dList));
